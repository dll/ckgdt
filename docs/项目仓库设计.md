...
| 类别 | 位置 | 职责 | 谁写 | 谁读 |
|------|------|------|------|------|
| **代码库** | Gitee `chzcldl/mad-kgdt`（主）<br>GitHub `dll/mad-kgdt`（镜像） | 应用源码、版本发布、Web 部署(gh-pages) | 开发者 | 开发者 / CI |
...
> 旧的中心仓库 `chzcldl/mad-kgdt` 已**弃用**（曾把全体学生数据集中到 `sync/students/`，导致仓库膨胀超配额）。
...
flowchart TB
    subgraph DEV["开发侧"]
        CODE["代码库<br/>Gitee chzcldl/mad-kgdt（主）<br/>GitHub dll/mad-kgdt（镜像 + gh-pages + Release）"]
    end
...
```
                    代码库 chzcldl/mad-kgdt (+ GitHub dll/mad-kgdt 镜像)
                              │ 构建发布
                 ┌────────────┴────────────┐
            学生端 App                  教师端 App
...
- **GitHub `dll/mad-kgdt`**：镜像，承载 `gh-pages`（Web 站 `https://dll.github.io/mad-kgdt/`）与 GitHub Release 资产。
...
| 维度 | 旧（已废弃） | 新（当前） |
|------|--------------|------------|
| 学生数据位置 | 中心仓 `chzcldl/mad-kgdt:sync/students/{学号}.json` | 各组仓库 `chzuczldl/cg*-*:mad/{学号}.json` |
...
> 旧 `sync/` 数据已迁至 `chzcldl/mad-data` 的 `同步/sync`，并从 `mad-fd` 历史（已迁移至 mad-kgdt）（已迁移至 mad-kgdt）中清除。
