...
> **当前 CI 状态（2026-06-01，第十五轮审核结束）**：`.github/workflows/ci.yml` 的
> `build-ios` job 跑 `flutter build ipa --release --no-codesign`，**只验证编译通过、不签名**
> ——产物无法直接装真机。要真机安装/分发必须按第三章配证书。
> iOS 工程审核已完成——`Podfile`/`xcscheme`/`Info.plist`/CI/技能/文档七项缺陷均已修复。
...
| Podfile | ✅ | 标准 Flutter 3.x Podfile，`platform :ios, '13.0'`，`post_install` 强制 `IPHONEOS_DEPLOYMENT_TARGET = 13.0` |
| 隐私描述 | ✅ | `NSCameraUsageDescription` / `NSMicrophoneUsageDescription` / `NSLocalNetworkUsageDescription` / `NSPhotoLibraryUsageDescription` 四权限全覆盖 |
| GPU 验证模式 | ✅ | `Runner.xcscheme` 的 `enableGPUValidationMode` 已设为 `"0"`（第十四轮修复） |
| AppIcon | ✅ | 16 张图标覆盖 iPhone + iPad 全尺寸（20/29/40/60/76/83.5/1024pt） |
...
| Bundle ID 一致性 | ✅ | `cn.edu.chzu.madkg` — 文档/`project.pbxproj`/`Info.plist`/`build-ios` 技能**四方一致**（第十三轮修正） |
| 部署目标一致性 | ✅ | iOS 13.0 — `Podfile`/`project.pbxproj`（Debug/Release/Profile）/**五处一致** |
| `build-ios` 技能 | ✅ | Bundle ID + 最低版本已在第十四轮前修好；本轮补 `ExportOptions.plist` 创建说明 + `--export-method` 替代方案 |
| CI `deploy-web` 提交消息 | ✅ | 已移除 `v0.12.0` 硬编码，改为通用自动消息（第十五轮修复） |

### 8.2 第十三～十五轮审计修复汇总

| 轮次 | 修复项 | 详情 |
|:----:|--------|------|
| 13 | Podfile 从无到有 | 18 个 iOS 原生插件依赖 CocoaPods，缺失导致完全无法编译 |
| 13 | Bundle ID 文档纠错 | `com.madkg.app` → `cn.edu.chzu.madkg`（以 `PRODUCT_BUNDLE_IDENTIFIER` 为准） |
| 13 | 最低版本纠错 | 技能 12.0 → 13.0（以 `Podfile` + `IPHONEOS_DEPLOYMENT_TARGET` 为准） |
| 14 | `enableGPUValidationMode` | `"1"` → `"0"`，关闭 Metal GPU Frame Capture 调试开销 |
| 14 | `NSPhotoLibraryUsageDescription` | 补隐私描述，作品/实验报告上传访问相册 |
| 15 | CI `deploy-web` 硬编码版本 | 移除 `v0.12.0` 硬编码 |
| 15 | `build-ios` 技能 ExportOptions.plist | 补创建说明 + `--export-method` 免文件方案 |
| 15 | `build-ios` 技能版本引用 | `v0.13→v0.14` 硬编码 → 通用"与 pubspec 对齐" |
...
- [ ] Apple Developer 账号 + 证书就绪后：创建 `ios/ExportOptions.plist` 或启用 `ci.yml` 签名步骤
