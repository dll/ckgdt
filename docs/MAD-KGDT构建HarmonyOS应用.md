...
安装后修改 `build_ohos.bat` 中对应的环境变量（若实际路径不同）：
...
在标准 Flutter 上 `dart:ui` 实例方法优先；在 OHOS 上 instance method 不存在，extension 兜底。该项目已导入此兼容层（76 个文件），但实际运行时全部走 `withOpacity`（补丁已替换），extension 仅用于预防将来新增的 `withValues` 调用。
...
大小约 72.3 MB（release，含 arm64-v8a native 库）。
...
| `lib/core/constants/color_ohos_compat.dart` | withValues 扩展兜底 | Dart 编译器 |
...
- [ ] `build_ohos.bat` 中 PATH 变量与实际安装路径一致
