...
target: 项目仓库 osgisOne/mad-fd（HEAD @56987e81d）
...
**严重性升级为"已泄露"而非"将泄露"**：`git log -S` 追踪显示该 key 自 commit `0fae0c0bf`（"add AI generation system"）引入，历经 v0.9.0 等多个版本一直在 master，而仓库**已公开镜像到 GitHub `dll/mad-fd` 并部署 GitHub Pages**。也就是说这个 key 早已可被任何人从公开源码中提取——它能直接调用 DeepSeek API，产生**真实账单**与**额度滥用**风险，且可能被爬虫自动收割。
