...
### AI Key 策略（开发阶段 vs 正式发布）

**开发/测试阶段**（当前）：不要求用户填写 AI Key，系统自动提供内置试用：

| 用户角色 | AI Key 行为 | 说明 |
|---------|------------|------|
| **管理员/教师** | 自动使用数据库内置 DeepSeek Key | 无需任何配置，登录即可使用全部 AI 功能 |
| **学生** | 一次性 1M Token 试用额度 | 额度用完后提示自行配置 Key；可在「设置 → AI 配置」填写自己的 Key 解除限制 |
| **语音导航** | 讯飞凭据未配置时静默降级 | 不阻断用户，语音功能不可用但其他功能正常 |

**正式发布阶段**（平台对外部署时）：
- 所有用户（含管理员/教师）必须填写自己的 AI Key
- 移除数据库内置试用 Key
- 移除学生试用额度机制
- 语音导航需用户自行配置讯飞凭据
