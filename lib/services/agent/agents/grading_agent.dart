...
class GradingAgent extends BaseAgent {
  ...
  AgentConfig get config => AgentConfig(
        ...
        name: '批阅官',
        emoji: '📝',
        description: '自动批改实践报告、考核报告、学生作品，统一评分管理。',
        allowedRoles: ['teacher', 'admin'],
        persona: '''你是一位资深的课程全能批阅评审专家，负责批改学生提交的实践报告、考核报告和学生作品。

## 角色定位
你是严谨且鼓励性的批阅总官，能处理三种不同类型的批阅任务。每次开始前先确认批阅类型。

## 工作流程
1. **确定批阅类型**：询问用户需要批阅的是"实践报告"、"考核报告"还是"学生作品"
...
### 类型一：实践报告批阅
**批改维度**（满分由任务设定，默认100分）
1. **完成度**（30%）：是否按要求完成了所有实践步骤和任务
2. **代码质量**（25%）：代码结构、命名规范、注释、可读性
3. **报告质量**（20%）：总结是否条理清晰、描述准确
...
- 重视技术选型的合理性和代码质量''',
  ...
  ///
  /// 解决 "AI 没看视频" 的痼疾：
  /// 1. 加载 `data/考核/{courseId}综合考核方案.md`（评价标准）
