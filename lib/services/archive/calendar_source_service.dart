...
/// 教学日历真实来源。
///
/// 和教学任务书一致：优先读取本地已保存的校历网页源文件；没有可解析缓存时，
/// UI 可打开 WebVPN/校历页面，由教师登录后提取当前页面 HTML。
class CalendarSourceService {
  CalendarSourceService._();

  static const webVpnHomeUrl = 'https://webvpn.chzu.edu.cn/';
  static const _sourceNameToken = 'schcalendar';
  ...
  static const _sourcePrefix = '06-';
  static const _sourceExtensions = ['.mhtml', '.mht', '.html', '.htm'];

  static Future<CalendarParseResult?> parseBestStoredSource({
    ...
    for (final file in storedSourceCandidates(
      ...
      final parsed = ArchiveImporters.parseCalendar(raw, now: now);
      if (parsed == null) continue;
      return CalendarParseResult(
        markdown: parsed,
        sourcePath: file.path,
        sourceLabel: '已保存校历网页源文件',
      );
    }
    return null;
  }
  ...
  static List<File> storedSourceCandidates({
    ...
    void addIfCandidate(File file) {
      ...
      if (!hasToken && !isNumberedSource) return;
  ...
  static Future<File> saveFetchedHtml({
    ...
    final file = File(
      p.join(dir.path, '06-$_sourceNameToken.fetched.$stamp.html'),
    );
    await file.writeAsString(html);
    return file;
  }

  static Future<String> preferredCalendarUrl({
    required String periodKey,
  }) async {
    for (final file in storedSourceCandidates(
      periodKey: periodKey,
      ...
      if (!await file.exists()) continue;
      final url = _sourceUrlFromMhtml(await file.readAsString());
      if (url != null) return url;
    }
    return webVpnHomeUrl;
  }

  static String? _sourceUrlFromMhtml(String raw) {
    final matches = RegExp(
      r'(?:Snapshot-Content-Location|Content-Location):\s*(\S+)',
      caseSensitive: false,
    ).allMatches(raw);
    for (final match in matches) {
      final url = match.group(1)?.trim();
      if (url == null || url.isEmpty) continue;
      if (url.toLowerCase().contains('schcalendar')) return url;
    }
    return null;
  }
