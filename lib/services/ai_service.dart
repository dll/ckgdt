    ...
    if (!isTeacherOrAdmin) {
      ...
      if (trialEnabled) {
        ...
        if (remainingCalls <= 0 || remainingTokens <= 0) {
          throw '免费 AI 试用额度已用完（累计 ${remaining['remainingCalls'] < 0 ? "∞" : "${remaining['remainingCalls']}"} 次调用 / ${remaining['remainingTokens'] < 0 ? "∞" : "${remaining['remainingTokens']}"} Token）。'
