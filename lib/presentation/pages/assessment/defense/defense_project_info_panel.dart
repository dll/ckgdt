...
class DefenseProjectInfoPanel extends StatefulWidget {
  ...
  const DefenseProjectInfoPanel(
      {super.key, required this.userId, this.compact = false});

  @override
  State<DefenseProjectInfoPanel> createState() =>
      _DefenseProjectInfoPanelState();
}
...
class _DefenseProjectInfoPanelState extends State<DefenseProjectInfoPanel> {
  ...
  Widget build(BuildContext context) {
    ...
    final scores =
        (cover['scores'] as Map?)?.cast<String, int>() ?? const <String, int>{};
    ...
    return _wrap(Column(
      ...
      children: [
        const Row(children: [
          Icon(Icons.assignment_ind, size: 16, color: NoirTokens.accent),
          SizedBox(width: 6),
          Text('答辩项目',
              style: TextStyle(
                  fontSize: 13,
        ...
        if (scores.isNotEmpty) ...[
          ...
          Wrap(
              ...
              children: scores.entries
                  .map((e) => _scoreChip(e.key, e.value))
                  .toList()),
        ],
      ],
    ));
