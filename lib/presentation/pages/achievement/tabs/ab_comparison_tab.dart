...
class _AbComparisonTabState extends State<AbComparisonTab> {
  ...
  Future<void> _loadScores() async {
    ...
    try {
      ...
      final rows = await widget.achievementDao.getCourseObjectives(courseName);
      final activeCourse = await _courseContext.getActiveCourse();
      final cfg =
          AchievementConfig.fromObjectiveRows(rows, course: activeCourse);
  ...
  Widget _buildSingleSelector(
    ...
    return Column(
      ...
      children: [
        Text(label,
            style: TextStyle(
                fontWeight: FontWeight.bold, fontSize: 12, color: primary)),
  ...
  Widget _buildComparisonMetrics(Color primary) {
    ...
    Widget metricCard(
        String label, String aVal, String bVal, String delta, Color dColor) {
      return Expanded(
        child: Container(
                ...
                RadarChartData(
                  ...
                  dataSets: [
                    RadarDataSet(
                      ...
                      entryRadius: 3,
                      dataEntries: [
                        for (final v in aVals)
                          RadarEntry(value: v.clamp(0.0, 1.0))
                      ],
                    ),
                    RadarDataSet(
