...
const _kDefaultWeights = [0.10, 0.20, 0.30, 0.40];
...
class _ReportTabState extends State<_ReportTab> {
  ...
  Future<void> _generateMarkdownReport() async {
    ...
    try {
      ...
      buffer.writeln('|---------------|---------|------|------------|');
      for (int i = 0; i < 4; i++) {
        buffer.writeln('| 指标点${objIndicators[i]} | ${_kObjectiveNames[i]} | ${_kDefaultWeights[i].toStringAsFixed(2)} | ${objDescFull[i]} |');
      }
      buffer.writeln();
      ...
      buffer.writeln('|----------|------|-----------|-----------------|-----------------|-----------------|');
      for (int i = 0; i < 4; i++) {
        buffer.writeln('| ${_kObjectiveNames[i]} | ${_kDefaultWeights[i].toStringAsFixed(2)} | 指标点${objIndicators[i]} | ${objMarks[i]} | ${objMarks[i]} | ${objMarks[i]} |');
      }
      ...
      for (int i = 0; i < 4; i++) {
        ...
        buffer.writeln('| ${_kObjectiveNames[i]} | $content | 全面掌握，表现突出 | 较好掌握，表现良好 | 基本掌握，表现一般 | 未能掌握，需要改进 |');
      }
      buffer.writeln();
      ...
      for (int i = 0; i < 4; i++) {
        ...
        buffer.writeln('| ${_kObjectiveNames[i]} | $expItem | 独立完成，结果正确 | 基本完成，结果较好 | 能够完成，有少量错误 | 无法完成或错误较多 |');
      }
      buffer.writeln();
      ...
      for (int i = 0; i < 4; i++) {
        ...
        buffer.writeln('| ${_kObjectiveNames[i]} | $examContent | ${objMarks[i]} |');
      }
      ...
      for (int i = 0; i < 4; i++) {
        ...
        for (int j = 0; j < 3; j++) {
          ...
          if (i == 2 && j == 0) {
            // 课程目标3无平时成绩
            buffer.writeln('| ${isFirstRow ? _kObjectiveNames[i] : ''} | ${isFirstRow ? _kDefaultWeights[i].toStringAsFixed(2) : ''} | ${assessNames[j]} | ${assessFullMarks[j]} | — | — | ${assessWeights[j]} | ${isFirstRow ? objCombined.toStringAsFixed(4) : ''} | ${isFirstRow ? '指标点${objIndicators[i]}' : ''} | ${isFirstRow ? objCombined.toStringAsFixed(4) : ''} |');
            continue;
          }
          final ach = assessMaps[j]['obj${i + 1}'] ?? 0.0;
          final avgScore = ach * assessFullMarks[j];
          buffer.writeln('| ${isFirstRow ? _kObjectiveNames[i] : ''} | ${isFirstRow ? _kDefaultWeights[i].toStringAsFixed(2) : ''} | ${assessNames[j]} | ${assessFullMarks[j]} | ${avgScore.toStringAsFixed(2)} | ${ach.toStringAsFixed(4)} | ${assessWeights[j]} | ${isFirstRow ? objCombined.toStringAsFixed(4) : ''} | ${isFirstRow ? '指标点${objIndicators[i]}' : ''} | ${isFirstRow ? objCombined.toStringAsFixed(4) : ''} |');
        }
      }
      buffer.writeln();
      ...
      for (int i = 0; i < 4; i++) {
        ...
        buffer.writeln('| ${_kObjectiveNames[i]}（权重${(_kDefaultWeights[i] * 100).toStringAsFixed(0)}%） | ${a.toStringAsFixed(4)} | 0.60 | ${a >= 0.60 ? '达成' : '未达成'} |');
      }
      ...
      for (int idx = 0; idx < scores.length; idx++) {
        ...
        final wt = a1 * _kDefaultWeights[0] + a2 * _kDefaultWeights[1] + a3 * _kDefaultWeights[2] + a4 * _kDefaultWeights[3];
      ...
      for (int i = 0; i < 4; i++) {
        ...
        buffer.writeln('**${_kObjectiveNames[i]}**（达成度：${a.toStringAsFixed(4)}，$perf）');
        buffer.writeln();
      ...
      if (_surveySummary?['hasSurveyData'] == true) {
        ...
        buffer.writeln();
        buffer.writeln('- 学生在${_kObjectiveNames[sortedIdx[0]]}方面表现最好（${_objectiveAchievements[sortedIdx[0]].toStringAsFixed(4)}）');
        buffer.writeln('- ${_kObjectiveNames[sortedIdx[3]]}方面相对较弱（${_objectiveAchievements[sortedIdx[3]].toStringAsFixed(4)}）');
        buffer.writeln();
      ...
      for (int i = 0; i < 4; i++) {
        final a = _objectiveAchievements[i];
        if (a < 0.60) {
          buffer.writeln('${i + 1}. **${_kObjectiveNames[i]}（${a.toStringAsFixed(4)}，未达标）**：大幅增加相关课时和实践环节，增设单元测验，对低分学生进行一对一辅导');
        } else if (a < 0.70) {
          buffer.writeln('${i + 1}. ${_kObjectiveNames[i]}（${a.toStringAsFixed(4)}）：加大跨平台开发方案的对比分析训练，增加知识图谱创建，补充测验题目');
        } else {
          buffer.writeln('${i + 1}. ${_kObjectiveNames[i]}（${a.toStringAsFixed(4)}）：保持现有教学节奏，适当提高考核难度，培养学生创新能力');
        }
      }
      buffer.writeln();
  ...
  void _showReportDialog(String reportText) {
    showDialog(
      context: context,
      builder: (ctx) => _ReportPreviewDialog(reportText: reportText),
    );
  }
        ...
        pw.MultiPage(
          ...
          build: (context) => [
              ...
              data: [
                for (int i = 0; i < 4; i++)
                  for (int j = 0; j < 3; j++) ...[
                    [
                      j == 0 ? '课程目标${i + 1}' : '',
                      j == 0 ? _kDefaultWeights[i].toStringAsFixed(2) : '',
                      ['平时成绩', '实验成绩', '期末成绩'][j],
                      ['20', '30', '50'][j],
                      (i == 2 && j == 0) ? '—' :
                          ((([pingshiAvg, experimentAvg, examAvg][j]['obj${i + 1}'] ?? 0.0) * [20, 30, 50][j]) as double).toStringAsFixed(2),
                      (i == 2 && j == 0) ? '—' :
            ...
            pw.TableHelper.fromTextArray(
              ...
              data: [
                for (int i = 0; i < 4; i++) [
                  '课程目标${i + 1}（权重${(_kDefaultWeights[i] * 100).toStringAsFixed(0)}%）',
...
/// 报告预览对话框 — 支持渲染预览和源码两种视图
class _ReportPreviewDialog extends StatefulWidget {
  final String reportText;
  const _ReportPreviewDialog({required this.reportText});
