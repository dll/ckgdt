    ...
    try {
      ...
      try {
        ...
        if (remoteTasks != null && remoteTasks.isNotEmpty) {
          ...
          remoteDone = true;
          debugPrint(
              'LabTaskDao: Loaded ${remoteTasks.length} tasks from Gitee');
        }
      } catch (e) {
  ...
  Future<void> _insertTasksFromRemote(
    ...
    for (final task in remoteTasks) {
      ...
      await db.insert('lab_tasks', {
        ...
        'due_date':
            DateTime.now().add(Duration(days: dueOffset)).toIso8601String(),
        'status': 'active',
        'creator_id': '206004',
        'created_at': now,
        'updated_at': now,
  ...
  Future<void> _insertHardcodedTasks(Database db) async {
    ...
    final tasks = [
      ...
      },
      {
        ...
        'chapter': '第2章',
        'description':
            'Android方向：使用 Kotlin 实现"智慧校园"登录页面（EditText输入验证、Activity跳转与数据传递）。'
      ...
      },
      {
        ...
        'max_score': 100,
        'due_date':
            DateTime.now().add(const Duration(days: 28)).toIso8601String(),
      },
      {
        'title': '实验四 微信小程序开发',
        'chapter': '第4章',
        ...
        'max_score': 100,
        'due_date':
            DateTime.now().add(const Duration(days: 35)).toIso8601String(),
      },
      {
        'title': '实验五 鸿蒙多端应用开发',
        'chapter': '第5章',
        ...
        'max_score': 100,
        'due_date':
            DateTime.now().add(const Duration(days: 42)).toIso8601String(),
      },
      {
        'title': '实验六 跨平台综合项目实战',
        'chapter': '第6章',
        ...
        'max_score': 100,
        'due_date':
            DateTime.now().add(const Duration(days: 56)).toIso8601String(),
      },
    ];
